...
1. A general **External Skill Integration Procedure** — the checklist issue #82 asked for, so evaluating and installing a new third-party skill or skill bundle stops being re-derived from scratch each time (has happened 5+ times: mattpocock, taste-skill, make-interfaces-feel-better, silk-design, pg-aiguide).
...
**Placement:** in `skills/claude-infra-reference/SKILL.md`, after `## Browser Automation` and before the first existing worked-example section (`## External Skill Integrations (mattpocock/skills)`) — it documents the procedure those sections already followed by hand.
...
3. **Choose the install mechanism** — decision guide built from the 3 mechanisms already in use, each confirmed against a real prior integration:
   - Repo has `.claude-plugin/marketplace.json` → `claude plugin marketplace add <repo>` then `claude plugin install <name>@<name>` (used for `refero` and `agent-stylebooks`).
...
- Retrofitting the procedure onto integrations that predate it (mattpocock, taste-skill, make-interfaces-feel-better, silk-design, pg-aiguide) beyond the tiebreak-callout reformat in item 2 above.
