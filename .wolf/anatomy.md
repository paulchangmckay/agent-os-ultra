...
- `infrastructure-layer.md` (~889 tok)
- `knowledge-graph.md` (~266 tok)
- `new-project-bootstrap.md` (~414 tok)
- `personal-knowledge-layer.md` (~539 tok)
...
- `ba.md` — ba (~6228 tok)
- `bloat-audit.md` — Tags (~413 tok)
- `brand.md` — Brand Skill — Paul McKay Personal Brand (~1168 tok)
- `claude-infra-reference.md` — Claude Infra Reference (~1603 tok)
- `dataviz.md` — DataViz Skill — Executive Data Visualization (~1997 tok)
- `debt-ledger.md` — Scan (~202 tok)
- `divergent-ideation.md` — Divergent Ideation (~732 tok)
- `exec-dashboard.md` — Executive Dashboard Skill (~5490 tok)
- `github-issue-first.md` — GitHub Issue First (~1073 tok)
- `grill-me.md` (~31 tok)
- `grilling.md` (~168 tok)
- `handoff.md` (~241 tok)
- `html-export.md` — HTML Export Skill — Playwright Screenshot & PDF (~725 tok)
- `index.md` — Skills Reference (~65 tok)
- `issue-backlog-cycle.md` — Issue Backlog Cycle (~1729 tok)
- `model-routing.md` — Model Routing (~517 tok)
- `receiving-code-review.md` — Code Review Reception (~1588 tok)
- `session-reflect.md` — Session Reflect Skill (~1597 tok)
- `using-git-worktrees.md` — Using Git Worktrees (~1868 tok)
...
- `caching.md` — Caching Strategy (~1777 tok)
- `compliance.md` — Security & Compliance Frameworks (NIST CSF 2.0 + SSDF, OWASP, SOC 2, Well-Architected) (~7087 tok)
- `containers-and-orchestration.md` — Docker & Kubernetes Standards (~5122 tok)
- `data-protection.md` — Data Protection & Privacy (GDPR / UK-GDPR / CCPA) (~4385 tok)
- `databases.md` — Database Standards (Postgres / Supabase, BigQuery, SQLite) (~6754 tok)
- `debugging.md` — Systematic Debugging (root cause, not symptom) (~2639 tok)
- `dev-environment-isolation.md` — Dev-Environment Isolation & Sandboxing (~3793 tok)
- `dev-environments.md` — IDE & Dev-Environment Standards (VS Code, Xcode, Google Antigravity) (~4340 tok)
- `diagrams-and-visual-docs.md` — Diagrams & Visual Documentation (~4027 tok)
- `disaster-recovery.md` — Disaster Recovery, Backups & Restore Drills (~3475 tok)
- `engineering-workflow.md` — Engineering Workflow (spec → plan → build → verify) (~4191 tok)
- `foss-adoption.md` — FOSS Adoption: The "Secure AND Tested" Gate (~2029 tok)
- `frontend-web-security.md` — Frontend / Web-App Security (~2250 tok)
- `gcp.md` — Google Cloud Platform Standards (~6288 tok)
- `github-actions.md` — GitHub Actions CI/CD Standards (~3526 tok)
- `github-teams.md` — GitHub Team Workflows (solo + agents -> human team) (~3761 tok)
- `google-apps-script.md` — Google Apps Script Discipline (~3622 tok)
- `iac-terraform.md` — Infrastructure as Code (Terraform on GCP) (~4837 tok)
- `javascript-and-typescript.md` — JavaScript & TypeScript Standards (~3548 tok)
- `local-and-agentic-ai-tools.md` — Local & Agentic AI Tooling (~2984 tok)
- `logging-and-monitoring.md` — Log Location, Rotation & Unattended-Job Monitoring (~2032 tok)
- `macos-app-bundles.md` — macOS App Bundle Standards (~1723 tok)
- `multi-agent-coordination.md` — Multi-Agent & Shared-Repo Coordination (~2030 tok)
- `my-environment.template.md` — My Environment Profile (template) (~1068 tok)
- `observability-and-incident-response.md` — Observability & Incident Response (SRE) (~5612 tok)
- `package-managers.md` — Package Manager Standards (Homebrew, npm, mas) (~4150 tok)
- `python-typing-and-packaging.md` — Python Typing & Single-File→Package Layout (~831 tok)
- `python-web-apis.md` — Python Web API Standards (FastAPI / Uvicorn / psycopg) (~3300 tok)
- `resilience-engineering.md` — Resilience Engineering (designing software to degrade, not die) (~2741 tok)
- `scalability-and-system-design.md` — Scalability & System Design (the "-ilities") (~2101 tok)
- `secrets-and-key-rotation.md` — Secrets & Key Rotation Lifecycle (~2591 tok)
- `secure-data-processing.md` — Secure Data Processing for Commercial / Multi-Tenant Apps (~3439 tok)
- `standards-authoring.md` — Standards Authoring: distill sprawling conventions into checkable rules (~1453 tok)
- `testing-single-file.md` — Testing strategy for single-file scripts with module-level side effects (~440 tok)
- `testing.md` — Testing Strategy (strict, enforced gates) (~4711 tok)
...
- `defense-in-depth.md` — Defense-in-Depth Validation (~921 tok)
- `index.md` — Systematic Debugging (~2467 tok)
- `root-cause-tracing.md` — Root Cause Tracing (~1337 tok)
- `test-academic.md` — Academic Test: Systematic Debugging Skill (~172 tok)
- `test-pressure-1.md` — Pressure Test 1: Emergency Production Fix (~483 tok)
...
- `LEARNING-RECORD-FORMAT.md` — Learning Record Format (~700 tok)
...
- `2026-07-21-docs-site-public-launch-design.md` — Publish the docs-site publicly via GitHub Pages (~2395 tok)
...
- `2026-07-27-agent-team-architect-design.md` — agent-team-architect skill (~5019 tok)
...
- `cerebrum-write-guard.js` — PreToolUse hook (matcher: Edit|Write): warns if cerebrum.md changed on (~563 tok)
- `cerebrum-write-guard.test.js` — SCRIPT: run, runPost, withTmpRepo (~1189 tok)
- `openwolf-cron-gate.js` — Exports runOpenwolfCron, archiveOldMemoryEntries, checkMemoryConsolidation, checkCerebrumReflection (~1714 tok)
- `openwolf-cron-gate.test.js` — SCRIPT: withTmpProject, makeFakeCron, run (~2296 tok)
- `plugin-health-check-gate.sh` — Runs plugin-health-check.js at most once per 24h, gated by a timestamp (~266 tok)
- `plugin-health-check-gate.test.js` — SCRIPT: run (~890 tok)
...
- `post-compact-observation.js` — PostCompact hook: deterministically records that a compaction happened, (~481 tok)
- `post-compact-observation.test.js` — SCRIPT: run, withTmpCwd (~651 tok)
...
- `post-write-batch-nudge.js` — PostToolUse hook (matcher: Write|Edit|MultiEdit): nudges toward logging (~665 tok)
- `post-write-batch-nudge.test.js` — SCRIPT: run, withTmpCwd, markerPath (~799 tok)
- `pre-compact-snapshot.sh` — PreCompact hook: stamps a compaction event in .wolf/cerebrum.md and reminds (~512 tok)
- `pre-principles-injection.js` — PreToolUse hook (matcher: Write|Edit|MultiEdit): injects (~468 tok)
- `pre-principles-injection.test.js` — SCRIPT: run, withTmpCwd (~762 tok)
...
- `prune-token-ledger.js` — Exports pruneTokenLedger (~867 tok)
- `prune-token-ledger.test.js` — SCRIPT: withTmpProject, run, makeLedger (~1108 tok)
- `session-start.sh` — Unified SessionStart hook: Superpowers bootstrap + OpenWolf anatomy injection (~1066 tok)
- `session-start.test.js` — SCRIPT: runWithCwd, withTmpCwd (~360 tok)
...
- `understand-anything-staleness.js` — SessionStart hook: reminds when a project's understand-anything knowledge (~566 tok)
- `understand-anything-staleness.test.js` — makeTempDir: initGitRepo, commitAll (~1846 tok)
- `worktree-path-guard.js` — PreToolUse hook (matcher: Edit|Write): warns when an absolute file_path (~729 tok)
- `worktree-path-guard.test.js` — SCRIPT: run, withWorktree (~625 tok)
...
- `setup-branch-protection.sh` — Reproducible branch-protection setup for a repo owned by this account. (~356 tok)
- `setup-branch-protection.test.js` — SCRIPT: withFakeGh (~646 tok)
- `submodule-transfer.sh` — Automates transferring a commit from one worktree's submodule clone to (~403 tok)
- `submodule-transfer.test.js` — SCRIPT: git (~575 tok)
...
- `qa-agent-guide.md` — QA Agent Guide (~738 tok)
- `skill-writing-checklist.md` — Skill-Writing Checklist (delta only) (~492 tok)
...
- `performance.md` — Performance (~756 tok)
- `SKILL.md` — Details that make interfaces feel better (~2950 tok)
...
- `.pre-commit-config.yaml` (~166 tok)
- `.version-bump.json` (~189 tok)
- `CLAUDE.md` — OpenWolf (~2268 tok)
- `CODE_OF_CONDUCT.md` — Contributor Covenant Code of Conduct (~1306 tok)
- `gemini-extension.json` (~54 tok)
- `GEMINI.md` (~23 tok)
...
- `2026-03-11-zero-dep-brainstorm-server-design.md` — Zero-Dependency Brainstorm Server (~1498 tok)
- `2026-03-23-codex-app-compatibility-design.md` — Codex App Compatibility: Worktree and Finishing Skill Adaptation (~3317 tok)
- `2026-04-06-worktree-rototill-design.md` — Worktree Rototill: Detect-and-Defer (~5008 tok)
- `2026-05-05-platform-neutral-config-refs-design.md` — Platform-neutral config-file references — Phase B design (~1215 tok)
- `2026-05-05-platform-neutral-prose-design.md` — Platform-neutral prose — Phase A design (~1498 tok)
- `2026-05-05-platform-neutral-readme-design.md` — Platform-neutral README ordering — Phase C design (~520 tok)
- `2026-05-06-lift-drill-into-evals-design.md` — Lift drill into superpowers as `evals/` — design (~5214 tok)
- `2026-06-09-sdd-task-scoped-review-dispatch-design.md` — SDD Task-Scoped Review Dispatch (~4441 tok)
- `2026-06-10-positive-instruction-redesign-design.md` — Positive-Instruction Redesign of Skill Guidance — Design Spec (~2630 tok)
- `2026-06-10-strict-cost-sdd-design.md` — Strict-Cost SDD — Design Spec (~3796 tok)
...
- `server.cjs` — API routes: GET (1 endpoints) (~6849 tok)
...
- `CREATION-LOG.md` — Creation Log: Systematic Debugging Skill (~1059 tok)
- `defense-in-depth.md` — Defense-in-Depth Validation (~912 tok)
- `find-polluter.sh` — /*.test.ts' (~434 tok)
- `root-cause-tracing.md` — Root Cause Tracing (~1327 tok)
- `SKILL.md` — Systematic Debugging (~2466 tok)
...
- `codex-tools.md` — Codex Tool Mapping (~978 tok)
- `copilot-tools.md` — Copilot CLI Tool Mapping (~640 tok)
...
- `persuasion-principles.md` — Persuasion Principles for Skill Design (~1469 tok)
- `render-graphs.js` — Render graphviz diagrams from a skill's SKILL.md to SVG files. (~1388 tok)
...
- `browser-launcher.test.js` — assert: test (~506 tok)
- `helper.test.js` — Tests for the injected browser client (helper.js). (~2354 tok)
